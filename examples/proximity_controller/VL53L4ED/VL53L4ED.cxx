...
VL53L4ED::VL53L4ED(const char* i2c_devPath, struct i2c_config_s* i2c_config, const char* xshutPin_devPath) : 
                                            _i2c_devPath(i2c_devPath), 
                                            _i2c_config(i2c_config), 
                                            _xshutPin_devPath(xshutPin_devPath)
{
    _xshutPin = GPIO();
    _i2c = I2C_Master();
}
...
bool VL53L4ED::VL53L4ED_On(void)
{
    if (_xshutPin_devPath != nullptr) {
    if (!_xshutPin.writePin(PinStatus::GPIO_HIGH))
        return false;
    }
    usleep(10000);
    return true;
...
bool VL53L4ED::VL53L4ED_Off(void)
{
    if (_xshutPin_devPath != nullptr) {
    if (!_xshutPin.writePin(PinStatus::GPIO_LOW))
        return false;
    }
    usleep(10000);
    return true;
...
bool VL53L4ED::init()
{
    ...
    if (!_i2c.setup(_i2c_devPath, _i2c_config)) {
    ...
    if (_xshutPin_devPath != nullptr) {
        ...
        if (!_xshutPin.setPinType(_xshutPin_devPath, GPIO_OUTPUT_PIN))
            return false;
    }


    //Initialize VL53L4ED satellite component.
    if (InitSensor() != VL53L4ED_ERROR_NONE)
        return false;


    if (VL53L4ED_StartRanging() != VL53L4ED_ERROR_NONE)
        return false;
...
VL53L4ED_Error VL53L4ED::InitSensor(uint8_t newAddr)
    {
...
void VL53L4ED::run()
{
    ...
    for (;;)
    {
        NewDataReady = 0;
        do {
        status = VL53L4ED_CheckForDataReady(&NewDataReady);
    } while (!NewDataReady);


    if ((!status) && (NewDataReady != 0)) {
        // (Mandatory) Clear HW interrupt to restart measurements
        VL53L4ED_ClearInterrupt();

        // Read measured distance. RangeStatus = 0 means valid data
        VL53L4ED_GetResult(&results);
        printf(report, sizeof(report), "Status = %3u, Distance = %5u mm, Signal = %6u kcps/spad\r\n",
...
VL53L4ED_Error VL53L4ED::VL53L4ED_SensorInit(uint8_t newAddr)
{
  VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
  uint8_t Addr, tmp;
